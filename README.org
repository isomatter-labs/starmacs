...
* User Code
In order to allow you to use a better editing environment when adding your /own/
code to Starmacs, the =usr= directory is added to the load-path. This means that
any file you add in =~.emacs.d/usr/= will we automatically evaluated as you
launch Emacs. This way, Emacs Lisp code can be edited in a dedicated buffer,
rather than in a source block in an org document
