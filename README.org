...
#+begin_src emacs-lisp
  ...
  (defun axiom/setup-fonts ()
    (setq axiom/fixed-pitch-font (if (member "IBM Plex Mono" (font-family-list))
                                     "IBM Plex Mono"
                                   "Courier New"))
    (setq axiom/fixed-pitch-slant (if (member "IBM Plex Mono" (font-family-list))
                                     'italic
                                   'normal))

    (setq axiom/variable-pitch-font (if (member "Crimson Pro" (font-family-list))
                                        "Crimson Pro"
                                      "Times New Roman"))


    (setq axiom/org-code-font (if (member "DSE Typewriter" (font-family-list))
                                  "DSE Typewriter"
                                "Courier New"))

    (set-face-attribute 'default nil :font axiom/fixed-pitch-font :slant axiom/fixed-pitch-slant :height axiom/fixed-pitch-height)
    (set-face-attribute 'fixed-pitch nil :font axiom/fixed-pitch-font :slant axiom/fixed-pitch-slant :height axiom/fixed-pitch-height)
...
#+begin_src emacs-lisp
  (defun writer-mode--num-format (numbering)
    "Alternative numbering format for org-num.
     ...
     etc.
    "
    (setq axiom/title-font (if (member "Magorian" (font-family-list))
                         "Magorian"
                       "Baskerville"))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (set-face-attribute 'org-document-title nil :inherit '(shadow axiom/title-font) :height 1.2)
    (set-face-attribute 'org-document-info nil :inherit '(shadow axiom/title-font) :height 1.1)

    (set-face-attribute 'org-block nil :foreground nil :font axiom/org-code-font :inherit '(axiom/org-code-font))
    (set-face-attribute 'org-code nil   :inherit '(shadow axiom/org-code-font))g
    (set-face-attribute 'org-table nil   :inherit '(shadow axiom/org-code-font))
...
* LSP Features
** DAP-Mode
=dap-mode= is the debugging interface that is bundled with =lsp-mode=, and thus requires it.
It is built using Microsoft's =DAP= protocol, in much the same way the =LSP= protocol is built upon.
...
** Eglot
=eglot= is a more minimal alternative to =lsp-mode=, and one that requires less configuration
to work as-expected.
