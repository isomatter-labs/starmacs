...
*** Theme
#+begin_src emacs-lisp
  (defvar starmacs/current-theme 'dark)
...
#+begin_src emacs-lisp
      ...
      (defvar starmacs/comment-face starmacs/fixed-pitch-font)

      (defvar starmacs/org-mode-code-font "SF Mono")
      ...
      (set-face-attribute 'font-lock-comment-face nil :slant 'italic :font starmacs/comment-face :inherit '(shadow))
...
#+begin_src emacs-lisp
  (use-package org
    ...
    (dolist (face '((org-level-1 . 1.30)
                    ...
                    (org-level-7 . 1.05)
                    (org-level-8 . 1.05)))
      (set-face-attribute (car face) nil :font starmacs/title-font :weight 'regular :height (cdr face)))

    (set-face-attribute 'org-document-title nil :font starmacs/title-font :height 1.50)
    (set-face-attribute 'org-document-info nil :font starmacs/title-font :inherit '(shadow) :height 1.20)

    (set-face-attribute 'org-block nil :foreground nil :font starmacs/org-mode-code-font :height 120 :inherit 'fixed-pitch)
    (set-face-attribute 'org-code nil   :font starmacs/org-mode-code-font :inherit '(shadow fixed-pitch))
    (set-face-attribute 'org-table nil   :font starmacs/org-mode-code-font :inherit '(shadow fixed-pitch))
    (set-face-attribute 'org-verbatim nil :font starmacs/org-mode-code-font :inherit '(shadow fixed-pitch)))
#+end_src
  ...
  (defun writer-mode--num-format (numbering)
    ...
    (if (= (length numbering) 1)
        (propertize (concat (mapconcat
                             #'number-to-string
                             numbering ".") " | " )
                    'face `(:family starmacs/title-font
